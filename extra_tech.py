'''
Author: your name
Date: 2021-05-18 23:45:15
LastEditTime: 2021-05-23 09:17:57
LastEditors: Please set LastEditors
Description: In User Settings Edit
FilePath: /dl_2021_hw3/dl_2021_hw3/extra_tech.py
'''
# coding: utf-8
import argparse
import math
...
def train(model,criterion,no_grad=False):
    model = model.cuda()
...
if __name__ == '__main__':
    ...
    criterion_smoothed = LabelSmoothLoss(smoothing=0.1)
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            ## we use smoothed loss to update parameters and compute hard loss for perplexity
            ## WARNING: DO set no_grad=True when computing hard loss to avoid repetitive updating 
            train_loss_smoothed = train(model,criterion_smoothed)
            train_loss_hard = train(model,criterion_hard,no_grad=True)
            print(f'training loss of epoch {epoch} is {train_loss_hard}')
            if args.model == 'Transformer':
                val_loss_smoothed, _ , _= evaluate(model, data_loader.val_data,criterion_smoothed)
                val_loss_hard, _ ,_ = evaluate(model, data_loader.val_data,criterion_hard)
            else:
                val_loss = evaluate(model, data_loader.val_data)
            
            print(f'validation loss of epoch {epoch} is {val_loss_hard}')
            train_loss_epoch.append(train_loss_hard)
            val_loss_epoch.append(val_loss_hard)
            print('-' * 89)
            print('| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.2f} | '
                'valid ppl {:8.2f}'.format(epoch, (time.time() - epoch_start_time),
                                            val_loss_hard, math.exp(val_loss_hard)))
            print('-' * 89)

            if val_loss_hard < best_val_loss:
                best_val_loss = val_loss_hard
                best_model = model
                torch.save(best_model, f'../checkpoints/best_{args.model}_extratech.pt')
            scheduler.step()
    ...
    plt.savefig(f'../training_curves/{args.model}_extratech_curves', bbox_inches='tight')

    ## evaluate best_model in test set
    if args.model == 'Transformer':
        test_loss , _ , _ = evaluate(best_model, data_loader.test_data, criterion_hard)
    else:
